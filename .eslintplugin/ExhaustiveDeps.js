...
/**
   copy of the `react-hooks/exhaustive-deps` eslint rule
   to detect if a dependency is of type `Dispatch<...>` or `(Mutabel)RefObject<...>`
   because those won't need to be reported as missing!
   */
module.exports = {
  "exhaustive-deps": {
  meta: {
  ...
  create(context) {
    ...
    function visitFunctionWithDependencies(
      ...
      function isStableKnownHookValue(resolved) {
        ...
        /** @type {ts.Type} */
        let tsType = services.getTypeAtLocation(def.node);

        if ( id.type === 'ArrayPattern' && isArray(resolved.identifiers))
          tsType = services.getTypeAtLocation(resolved.identifiers[0]);

        const symbol = tsType.aliasSymbol ?? tsType.symbol;
        const symbolName = symbol?.escapedName ?? symbol?.name ?? "KA";
        const parentName = symbol?.parent?.escapedName ?? "KA";
        if (parentName === "React") {
          if (symbolName.includes("RefObject") || symbolName === "Dispatch")
            return true;
        }
